...
<script>
	function deleteNotice() {
		location.href = "notice_delete?noticeno=${notices.noticeno}";
		return false;
	}
</script>
</head>
		...
		<div class="container">
			<div class="row block-9">
				<c:if test="${member.memberid eq 'admin'}">
					<form action="notice_update" method="post" id="signupForm1"
						class="bg-white p-5 contact-form">
						<div class="form-group">
							<label>조회수:&nbsp;&nbsp;${notices.visited}</label>
						</div>
						<div class="form-group">
							<label>Writer</label> <input type="text" class="form-control"
								placeholder="관리자" disabled="disabled"> <input
								type="hidden" class="form-control" name="noticeno"
								value="${notices.noticeno}">
						</div>
						<div class="form-group">
							<label>Notice Period&nbsp;&nbsp;</label> <input type="text"
								name="notice_date" value="${notices.notice_date}"
								class="form-control" />
						</div>
						<div class="form-group">
							<label>Title</label> <input name="noticesubject"
								id="noticesubject" value="${notices.noticesubject}"
								class="form-control" />
						</div>
						...
						<div class="form-group">
							...
							<textarea id="" cols="30" rows="7" class="form-control"
								id="noticecontent" name="noticecontent">${notices.noticecontent}</textarea>
						</div>
						<div class="form-group">
							...
							<input type="button" value="Delete"
								class="btn btn-primary py-3 px-5" style="float: left;"
								onclick="deleteNotice()">&nbsp; <input type="button"
								value="Go back" class="btn btn-primary py-3 px-5"
								style="float: right;" onclick="history.back(-1);">&nbsp;
						</div>
					</form>
				</c:if>
				<c:if test="${member.memberid != 'admin'}">
					<form class="bg-white p-5 contact-form">
						...
						<div class="form-group">
							<label>Content</label>
							<textarea id="" cols="30" rows="7" class="form-control"
								id="noticecontent" name="noticecontent" disabled="disabled">${notices.noticecontent}</textarea>
						</div>
						<div class="form-group">
							<input type="button"
								value="Go back" class="btn btn-primary py-3 px-5"
